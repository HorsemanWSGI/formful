from datetime import datetime

from tests.common import DummyPostData

from formful.fields import DateTimeField
from formful.form import Form
from formful.schema import Schema


class Fields(Schema):
    a = DateTimeField()
    b = DateTimeField(format="%Y-%m-%d %H:%M")
    c = DateTimeField(format="%-m/%-d/%Y %-I:%M")


def test_basic():
    d = datetime(2008, 5, 5, 4, 30, 0, 0)
    # Basic test with both inputs
    form = Form(fields=Fields)
    form.process(DummyPostData(
        a=["2008-05-05", "04:30:00"],
        b=["2008-05-05 04:30"],
        c=["5/5/2008 4:30"]
    ))
    assert form['a'].data == d
    assert form['a']() == (
        """<input id="a" name="a" type="datetime" value="2008-05-05 04:30:00">"""
    )
    assert form['b'].data == d
    assert form['b']() == (
        """<input id="b" name="b" type="datetime" value="2008-05-05 04:30">"""
    )
    assert form['c'].data == d
    assert form['c']() == (
        """<input id="c" name="c" type="datetime" value="5/5/2008 4:30">"""
    )
    assert form.validate()

    # Test with a missing input
    form.process(DummyPostData(a=["2008-05-05"]))
    assert not form.validate()
    assert form['a'].errors[0] == "Not a valid datetime value."

    form.process(a=d, b=d, c=d)
    assert form.validate()
    assert form['a']._value() == "2008-05-05 04:30:00"
    assert form['b']._value() == "2008-05-05 04:30"
    assert form['c']._value() == "5/5/2008 4:30"


def test_microseconds():
    d = datetime(2011, 5, 7, 3, 23, 14, 424200)
    form = Form(fields={
        'a': DateTimeField(format="%Y-%m-%d %H:%M:%S.%f")
    })
    form.process(DummyPostData(a=["2011-05-07 03:23:14.4242"]))
    assert d == form['a'].data


def test_multiple_formats():
    d = datetime(2020, 3, 4, 5, 6)
    form = Form(fields={
        'a': DateTimeField(format=["%Y-%m-%d %H:%M", "%Y%m%d%H%M"])
    })
    form.process(DummyPostData(a=["2020-03-04 05:06"]))
    assert d == form['a'].data

    form.process(DummyPostData(a=["202003040506"]))
    assert d == form['a'].data
