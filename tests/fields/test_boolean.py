...
def test_defaults():
    ...
    form = Form(fields=BooleanFields)
    form.process()
...
def test_rendering():
    form = Form(fields=BooleanFields)
    form.process(DummyPostData(bool2="x"))
...
def test_with_postdata():
    form = Form(fields=BooleanFields)
    form.process(DummyPostData(bool1=["a"]))
    assert form['bool1'].raw_data == ["a"]
    assert form['bool1'].data is True

    form.process(DummyPostData(bool1=["false"], bool2=["false"]))
    assert form['bool1'].data is False
    assert form['bool2'].data is True
...
def test_with_postdata_and_model():
    form = Form(fields=BooleanFields)
    form.process(DummyPostData(bool1=["y"]), obj=obj)
    assert form['bool1'].data is True
